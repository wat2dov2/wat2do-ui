#!/usr/bin/env python3
"""Reconcile Instagram following exports into production organizations.
...
organization as student-union affiliated.

Usage (from backend/):
  uv run python scripts/import_instagram_following_organizations.py \
    --archive utsg=/path/to/export.zip \
    --classification /tmp/utsg.jsonl \
    --plan /tmp/instagram-organizations.json

  uv run python scripts/import_instagram_following_organizations.py \
    --plan /tmp/instagram-organizations.json --apply --school utsg
"""
...
from core.tables import ORGANIZATIONS, SCHOOLS  # noqa: E402
...
def read_classifications(paths: list[Path]) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    for path in paths:
        for line_number, line in enumerate(path.read_text().splitlines(), start=1):
            ...
            if any(category not in ORGANIZATION_CATEGORIES for category in categories):
...
def _fetch_organizations(sb: Any, school_ids: dict[str, int]) -> list[dict[str, Any]]:
    ...
    while True:
        result = (
            ...
            .select("id,organization_name,school_id,categories,ig,organization_type,status")
...
def _new_candidate(
    ...
    if len(workbook_matches) == 1:
        ...
        return {
            "organization_name": str(row["name"])[:ORGANIZATION_NAME_MAX],
            "categories": row.get("categories") or [],
            "ig": handle,
            "organization_page": row.get("directory"),
    ...
    name = " ".join(str(classification.get("organization_name") or "").split())
    if not name:
        return None, "classification_missing_name"
    return {
        "organization_name": name[:ORGANIZATION_NAME_MAX],
...
def reconcile_planned_inserts(
    ...
    for row in inserts:
        by_school_name[(row["school"], normalize_name(row["organization_name"]))].append(row)
...
def build_plan(
    ...
    existing = _fetch_organizations(sb, {s: school_ids[s] for s in archives})
    ...
    for row in existing:
        ...
        by_name[(school, normalize_name(row["organization_name"]))].append(row)

    updates = [
        {
            "id": int(row["id"]),
            "school": slug_by_id[int(row["school_id"])],
            "organization_type": UNION_TYPES[slug_by_id[int(row["school_id"])]],
        }
        for row in existing
        if row.get("organization_type") != UNION_TYPES[slug_by_id[int(row["school_id"])]]
    ]
    ...
    for school, archive_path in sorted(archives.items()):
        for handle in sorted(read_following_handles(archive_path)):
            ...
            name_matches = by_name.get((school, normalize_name(candidate["organization_name"])), [])
            if len(name_matches) == 1:
                ...
                if update is None:
                    update = {
                        ...
                        "school": school,
                        "organization_type": UNION_TYPES[school],
                    }
...
def apply_plan(sb: Any, plan: dict[str, Any], school: str | None = None) -> set[str]:
    ...
    if selected_schools:
        school_ids = _fetch_schools(sb)
        current_rows = _fetch_organizations(
            sb,
    ...
    for row in updates:
        if set(row) == {"id", "school", "organization_type"}:
            simple_updates[(row["school"], row["organization_type"])].append(row["id"])
        else:
            detailed_updates.append(row)

    for (update_school, organization_type), ids in simple_updates.items():
        for start in range(0, len(ids), 200):
            sb.table(ORGANIZATIONS).update({"organization_type": organization_type}).in_(
    ...
    for row in detailed_updates:
        ...
        sb.table(ORGANIZATIONS).update(update_payload).eq("id", row["id"]).execute()
    ...
    for start in range(0, len(inserts), 200):
        ...
        sb.table(ORGANIZATIONS).insert(insert_payload).execute()
