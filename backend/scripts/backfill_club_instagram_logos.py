#!/usr/bin/env python3
"""Backfill permanent club logos from Instagram profile pictures.
...
prefix, and then persisted in ``clubs.logo_url``.

The operation is re-runnable. Clubs that already have ``logo_url`` are
...
def list_school_clubs(school: str) -> list[Any]:
    """Return every approved club for a school."""
    clubs: list[Any] = []
    while True:
        page, total = club_service.list_clubs(
            school=school,
            skip=len(clubs),
            limit=PAGE_SIZE,
        )
        clubs.extend(page)
        if not page or len(clubs) >= total:
            return clubs
...
def backfill_school(
    ...
    *,
    ...
    clubs = list_school_clubs(school)
    eligible = [club for club in clubs if club.ig and not club.logo_url]
    by_identifier: dict[str, list] = defaultdict(list)
    for club in eligible:
        by_identifier[normalize_instagram_identifier(club.ig)].append(club)
    ...
    for profile in profiles:
        matched = {
            ...
            for club in by_identifier.get(identifier, [])
        }
        if not matched:
            continue
        source_urls = _profile_picture_urls(profile)
        for club in matched.values():
            if club.id in updated_ids or club.id in failed_ids:
                continue
            if not source_urls:
                reason = profile.get("error") or "missing profile picture"
                print(f"  profile failed: {club.id} {club.club_name}: {reason}")
                failed_ids.add(club.id)
                continue
            permanent_url = None
            for source_url in source_urls:
                permanent_url = upload_image_from_url(
                    source_url,
                    bucket=BUCKET_CLUB_LOGOS,
                )
                if permanent_url is not None:
                    break
            if permanent_url is None:
                print(f"  image failed: {club.id} {club.club_name}")
                failed_ids.add(club.id)
                continue
            try:
                result = club_service.update_club(
                    club.id,
                    ClubUpdate(logo_url=permanent_url),
                )
                if result is None:
                    raise RuntimeError("club disappeared before update")
            except Exception as exc:  # noqa: BLE001 - continue the resumable batch
                _delete_uploaded_logo(permanent_url)
                print(
                    f"  database failed: {club.id} {club.club_name}: {exc}",
                    file=sys.stderr,
                )
                failed_ids.add(club.id)
                continue
            updated_ids.add(club.id)
            print(f"  updated: {club.id} {club.club_name}")

    unresolved_ids = {club.id for club in eligible} - updated_ids - failed_ids
    for club in eligible:
        if club.id in unresolved_ids:
...
def main() -> int:
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Call Apify, upload to S3, and update clubs",
    )
    parser.add_argument(
        "--profile-cache",
        type=Path,
    ...
    clubs = list_school_clubs(args.school)
    eligible = [club for club in clubs if club.ig and not club.logo_url]
    print(
        f"{args.school}: {len(clubs)} approved clubs, "
