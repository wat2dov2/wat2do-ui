...
from scripts import reconcile_duplicate_organizations as module
        ...
        "merges": [
            {
                ...
                "canonical": {
                    "id": 2,
                    "organization_name": "Best Club UTSC",
                    "ig": "best.club",
                },
                "duplicates": [
                    {
                        "id": 1,
                        "organization_name": "Best Club Student Chapter",
                        "ig": "bestclub.old",
                    }
                ],
            }
        ...
        "instagram_clears": [
            {
                ...
                "organization": {
                    "id": 3,
                    "organization_name": "Different Club",
                    "ig": "best.club",
                },
            }
        ],
...
def test_validate_plan_rejects_an_organization_in_multiple_actions():
    plan = _plan()
    plan["instagram_clears"][0]["organization"]["id"] = 1
...
def test_build_transaction_sql_moves_dependencies_and_guards_reviewed_rows():
    ...
    assert "Organization reconciliation plan no longer matches production" in sql
    assert "UPDATE events SET organization_id = 2 WHERE organization_id = 1" in sql
    assert "UPDATE positions SET organization_id = 2 WHERE organization_id = 1" in sql
    assert "NULLIF(canonical.organization_type, 'independent')" in sql
    assert "Organization merge 2 has conflicting organization_memberships rows" in sql
    assert "UPDATE organizations SET ig = NULL, logo_url = NULL WHERE id = 3" in sql
...
def test_delete_unreferenced_logos_preserves_referenced_objects(monkeypatch):
    ...
    path_from_url.assert_called_once_with(
        "https://wat2do.io/media/organization-logos/orphan.jpg",
        module.BUCKET_ORGANIZATION_LOGOS,
    )
