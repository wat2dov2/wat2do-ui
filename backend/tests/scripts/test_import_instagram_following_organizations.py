...
from scripts import import_instagram_following_organizations as module
...
def test_new_candidate_requires_high_confidence_for_agent_classification():
    classifications = {
        ("utsg", "clear.club"): {
            "action": "insert",
            "confidence": "high",
            "organization_name": "Clear Club",
            "categories": ["Community Service"],
        },
        ("utsg", "maybe.club"): {
            "action": "insert",
            "confidence": "medium",
            "organization_name": "Maybe Club",
            "categories": [],
        },
    }
...
def test_build_plan_freezes_existing_rows_and_reconciles_exact_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda sb, school_ids: [
            {
                "id": 10,
                "organization_name": "Existing Club",
                "school_id": 1,
                "categories": ["Business"],
                "ig": "existing.club",
            ...
            {
                "id": 11,
                "organization_name": "Attached Club",
                "school_id": 1,
                "categories": [],
                "ig": None,
                "organization_type": "independent",
    ...
    assert plan["updates"] == [
        {"id": 10, "school": "utsg", "organization_type": "utsu"},
        {
            "categories": ["Arts & Culture"],
            "id": 11,
            "ig": "attached.club",
            "school": "utsg",
            "organization_type": "utsu",
        },
    ]
    assert plan["inserts"] == [
        {
...
def test_reconcile_planned_inserts_holds_duplicate_names_and_cross_school_handles():
    inserts = [
        {"school": "utsg", "ig": "club.old", "organization_name": "Same Club"},
        {"school": "utsg", "ig": "club.new", "organization_name": "Same Club"},
        {"school": "utsg", "ig": "shared.club", "organization_name": "Shared Club"},
        {"school": "utsc", "ig": "shared.club", "organization_name": "Shared Club UTSC"},
        {"school": "utsg", "ig": "unique.club", "organization_name": "Unique Club"},
    ]
    reviews = []

    kept = module.reconcile_planned_inserts(inserts, reviews)

    assert kept == [{"school": "utsg", "ig": "unique.club", "organization_name": "Unique Club"}]
...
def test_apply_plan_writes_selected_school_and_revalidates_once(monkeypatch):
    ...
    monkeypatch.setattr(module, "_fetch_schools", lambda sb: {"utsg": 1})
    monkeypatch.setattr(module, "_fetch_organizations", lambda sb, school_ids: [])
    revalidate = MagicMock()
    monkeypatch.setattr(module.event_feed_revalidation_service, "revalidate_school", revalidate)
    plan = {
        "updates": [
            {"id": 1, "school": "utsg", "organization_type": "utsu"},
            {"id": 2, "school": "utsc", "organization_type": "scsu"},
        ],
        "inserts": [
            {
                "school": "utsg",
    ...
    assert query.update.call_args.args[0] == {"organization_type": "utsu"}
    query.in_.assert_called_once_with("id", [1])
    assert query.insert.call_args.args[0][0]["organization_name"] == "New Club"
