...
def test_list_school_organizations_reads_every_page(monkeypatch):
    organizations = [SimpleNamespace(id=index) for index in range(3)]

    def list_organizations(*, school, skip, limit):
        assert school == "mcmaster"
        assert limit == module.PAGE_SIZE
        return organizations[skip : skip + 2], len(organizations)
    ...
    assert list_school_organizations("mcmaster") == organizations
...
def test_backfill_school_reuses_profile_cache_without_apify(monkeypatch):
    ...
    monkeypatch.setattr(module, "list_school_organizations", lambda school: [organization])
    scraper = MagicMock()
    monkeypatch.setattr(module, "get_scraper", lambda: scraper)
