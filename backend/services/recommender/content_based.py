...
def get_content_scores(
    ...
    user_school = user.get("school") or ""
    is_first_year = user.get("is_first_year", False)
    ...
    # Get user's past interactions for org affinity
    ...
    for event in candidate_events:
        ...
        if matched_categories and cat in matched_categories:
            ...
            # No interests set — neutral score
            score += 0.2
        ...
        if dtstart:
            try:
                ...
                if hours_away < 0:
                    ...
                    score += 0.15  # Happening soon — max boost
                elif hours_away < 72:
                    score += 0.12
                elif hours_away < 168:
                    score += 0.08
...
def _compute_org_affinity(
    ...
    # Build event_id -> org lookup from candidates
    ...
    # Also need past events the user interacted with
