...
def test_recommendations_personalized_failure_falls_back_to_popular(
    ...
    fallback_recs = _mock_recs(1)
    monkeypatch.setattr(
        engine,
        "get_personalized_recommendations",
        MagicMock(side_effect=RuntimeError("stream dropped")),
    )
    monkeypatch.setattr(
        engine,
        "get_popular_recommendations",
        MagicMock(return_value=fallback_recs),
    )
    ...
    assert resp.status_code == 200
    assert resp.json() == [
        {"event_id": 1, "score": 0.9, "reason": "popular"},
    ]


def test_recommendations_popular_failure_returns_empty(client, monkeypatch):
    """Recommendations are non-critical; total fallback failure returns [] not 500."""
    from recommender.service import engine
