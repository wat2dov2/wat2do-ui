-- Drafts contain all candidates; the service enforces Meta's limit at publish time.
alter table public.instagram_publish_batches drop column if exists ai_model;
alter table public.instagram_publish_items drop constraint if exists instagram_publish_items_position_check;
alter table public.instagram_publish_items alter column position type integer;
alter table public.instagram_publish_items add constraint instagram_publish_items_position_check check (position > 0);
