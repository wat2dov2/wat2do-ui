...
MAX_RAW_DATA_BYTES = 32 * 1024


class ScrapedEventCreate(BaseModel):
    event_id: int | None = None
    source: str = Field(..., min_length=1, max_length=MAX_SCRAPED_SOURCE_LENGTH)
    raw_data: dict | None = None

    @field_validator("raw_data")
    @classmethod
    def _raw_data_size_cap(cls, v: dict | None) -> dict | None:
        """Reject ``raw_data`` payloads larger than ``MAX_RAW_DATA_BYTES``.
        ...
        if len(serialised.encode("utf-8")) > MAX_RAW_DATA_BYTES:
            raise ValueError(
                f"raw_data exceeds maximum size of {MAX_RAW_DATA_BYTES} bytes"
            )
        return v
