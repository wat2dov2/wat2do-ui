...
import { fetchClubEvents } from "@/features/events/api/events.api";
...
/**
 * Every event one club has run, past included, as a flat grid.
 *
 * The club page is the host's whole record, so there are no filters and
 ...
 */
export function ClubEventsGrid({
  clubId,
  school,
  initialEvents,
}: ClubEventsGridProps) {
  const { data: events, isPending } = useQuery({
    queryKey: queryKeys.events.byClub(clubId, school),
    queryFn: () => fetchClubEvents(clubId, school),
  ...
  const clubEvents = useMemo(() => {
    ...
    return currentTimeMs === null
      ? all
      : orderClubEvents(all, currentTimeMs);
  }, [currentTimeMs, events]);
  const selectedEvent = useMemo(
    () => clubEvents.find((event) => event.id === selectedEventId) ?? null,
