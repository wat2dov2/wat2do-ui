...
interface ClubCardFrameDimensions {
  w: number;
  h: number;
  cw: number;
  ch: number;
...
interface ClubCardFramePaths {
  borderOuter: string;
  borderCutout: string;
  clip: string;
}

const EMPTY_FRAME_PATHS: ClubCardFramePaths = { borderOuter: "", borderCutout: "", clip: "" };

function getClubCardFramePaths({
  w,
  h,
  cw,
  ch,
}: ClubCardFrameDimensions): ClubCardFramePaths {
...
export function useClubCardFrame() {
  const cardRef = useRef<HTMLDivElement>(null);
  const badgeRef = useRef<HTMLDivElement>(null);
  const [dimensions, setDimensions] = useState<ClubCardFrameDimensions>({
    w: 0,
    h: 0,
    cw: 0,
    ch: 0,
  ...
  const paths = useMemo(
    () => getClubCardFramePaths(dimensions),
    [dimensions],
  );
