...
import { getOrganizationPositions } from "@/features/positions/api/positions.api";
...
interface OrganizationPositionsGridProps {
...
export function OrganizationPositionsGrid({
  organizationId,
  school,
  initialPositions,
}: OrganizationPositionsGridProps) {
  const { t } = useTranslation();
  const { data: positions, isPending } = useQuery({
    queryKey: queryKeys.positions.byOrganization(organizationId, school),
    queryFn: () => getOrganizationPositions(organizationId, school),
  ...
  return (
    <>
      <PositionList
        ...
        emptyTitle={t("positions.organizationEmptyTitle")}
        emptyDescription={t("positions.organizationEmptyDescription")}
      />
