...
export function AdminPositionsPage({ onBack }: { onBack: () => void }) {
  ...
  const filters = { page, pageSize: ADMIN_ITEMS_PER_PAGE, search: submittedSearch, includeClosed: true };
        ...
        <TabsContent value="positions">
          <Stack gap={5}>
            ...
            {isPending ? <LoadingPage /> : isError ? <Button onClick={() => void refetch()}>{t("common.tryAgain")}</Button> : (
              <>
                <AdminTable count={data.total} label={t("positions.position", { count: data.total })} pagination={{ currentPage: page, totalPages: data.total_pages, onPageChange: setPage }} headers={[{ label: t("navigation.positions") }, { label: t("navigation.clubs") }, { label: t("schools.school") }, { label: t("common.actions") }]}>
